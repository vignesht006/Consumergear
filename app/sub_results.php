...
if(!isset($_GET['id'])){
	...
	$menu=mysqli_query($con,"select * from sub_categories where prev=".$_GET['id']);	
$products=mysqli_query($con,"select p.* , pi.main_image_path  from product_detail p left join product_images pi on p.id=pi.product_id where p.sub_category=".$_GET['id']);	
}
?>
...
$i=1;	
    while($row=mysqli_fetch_array($menu))
    {
		if($i<=8)
		{
    	$main.='<li class="dropdown"><a  href="sub_results.php?id="'.$row['id'].'">'.$row['name'].'</a>';
    	...
    	if(mysqli_num_rows($sub) > 0)
    	{
    	  $sub_menu='<ul class="dropdown-menu">';
    	   ...
    	   {
    	   	$sub_menu.='<li ><a href="sub_results.php?id="'.$r1['id'].'">'.$r1['name'].'</li>';
		...
		$sub_menu.='<li><a href="sub_results.php?id="'.$r1['id'].'" cat="'.$row['id'].'">'.$row['name'].'</a></li>';
   ...
   <?php
   $ban=mysqli_query($con,"select image_path from sub_categories where prev=".$_GET['id']);
